...
import { EDITOR_PADDING, CANVAS_MAX_WIDTH, CANVAS_MAX_HEIGHT } from '../constants/grid'
import {
  computeCellSize,
  drawCells,
  drawGridLines,
...
function onTouchStart(e: TouchEvent): void {
  ...
  const canvas = canvasRef.value
  if (!canvas) return
  const pos = screenToCell(touch, canvas, cellSize, EDITOR_PADDING, EDITOR_PADDING)
  const { r, c } = pos
  if (r < 0 || r >= gridHeight.value || c < 0 || c >= gridWidth.value) return
  paintValue = grid.value[r]?.[c] ? 0 : 1
  toggleCell(r, c, paintValue)
